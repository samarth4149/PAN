...
from utils import construct_feed_dict, write_log, support_dropout, generate_attr_labels
...
ap.add_argument("-n_output", "--node_output", type=int, default=206,
                help="number of output nodes")

ap.add_argument("-c_loss", "--combined_loss", type=bool, default=False,
                help="whether we use softmax loss")
...
ap.add_argument("-hi", "--hidden", type=int, nargs='+', default=[200,200],
...
placeholders = {
    ...
    'attribute_labels': tf.placeholder(tf.float32,shape=(None,attr_num))
}
...
model = CompatibilityGAE(placeholders,
                    ...
                    learning_rate=LR,
                    logging=True,
                    edge_c = args["combined_loss"],
                    batch_norm=BATCH_NORM,
                    wd=args['weight_decay'])


def process_label(c_labels,r_labels,operation='or'):
    if operation=="or":
        labels = np.logical_or(c_labels,r_labels)
    elif operation=="and":
        labels = np.logical_and(c_labels,r_labels)
    elif operation=="xor":
        labels = np.logical_xor(c_labels,r_labels)
    elif operation=="xnor":
        labels = np.logical_xor(c_labels,r_labels)
        labels = np.logical_not(labels) 
    row_sum = np.sum(labels,axis=1)
    empty_position = np.where(row_sum<0.5)[0]
    
    labels[empty_position,-1]=1
    
    return labels

attribute_list = []
# with open("data/polyvore_outfits/dataset/attribute_shuffle_list.txt",'r') as f:
with open("data/polyvore_outfits/dataset/attribute_list.txt",'r') as f:
    for ele in f:
        attribute_list.append(ele[:-1])

attribute_list = attribute_list[:]
#np.random.shuffle(attribute_list)

operation="or"
train_c_labels,train_r_labels = generate_attr_labels(train_c_indices,train_r_indices,n_output=attr_num,attribute_list=attribute_list)
train_attribute_labels = process_label(train_c_labels,train_r_labels,operation=operation)

val_c_labels,val_r_labels = generate_attr_labels(val_c_indices,val_r_indices, n_output=attr_num, attribute_list=attribute_list)
val_attribute_labels = process_label(val_c_labels,val_r_labels,operation=operation)

#test_c_labels,test_r_labels = generate_attr_labels(test_c_indices,test_r_indices,n_output=attr_num,attribute_list=attribute_list)
#test_attribute_labels = process_label(test_c_labels,test_r_labels)
#print(test_attribute_labels.shape)
...
train_feed_dict = construct_feed_dict(placeholders, train_features, train_support,
                    train_labels, train_attribute_labels, train_r_indices, train_c_indices, DO)

val_feed_dict = construct_feed_dict(placeholders, val_features, val_support,
                    val_labels, val_attribute_labels, val_r_indices, val_c_indices, 0., is_train=BN_AS_TRAIN)
#test_feed_dict = construct_feed_dict(placeholders, test_features, test_support,
#                    test_labels, test_attribute_labels, test_r_indices, test_c_indices, 0., is_train=BN_AS_TRAIN)
