...
class DataLoaderPolyvore(Dataloader):
    ...
    """
    def __init__(self):
        super(DataLoaderPolyvore, self).__init__(path='data/polyvore_outfits/dataset/')
    ...
    def init_phase(self, phase):
        ...
        # train_adj: adjacent matrix (means connections between matrix)
        node_features = sp.load_npz(feats_file)
        setattr(self, '{}_features'.format(phase), node_features)
        # train_features: ResNet50 image features (None,2048)
        
        # get lower tiangle(纠纷) of the adj matrix to avoid duplicate edges
    ...
    def get_phase(self, phase):
        ...
        # lower_adj: 'lower_train_adj'
        # get the positive edges
        
        pos_r_idx, pos_c_idx = lower_adj.nonzero()#row index & column index
        pos_labels = np.array(lower_adj[pos_r_idx, pos_c_idx]).squeeze()#the weight of each edge
        
        # split the positive edges into the ones used for evaluation and the ones used as message passing
        n_pos = pos_labels.shape[0] # number of positive edges,338488
        perm = list(range(n_pos))
        np.random.shuffle(perm)
        pos_labels, pos_r_idx, pos_c_idx = pos_labels[perm], pos_r_idx[perm], pos_c_idx[perm]#shuffle
        n_eval = int(n_pos/2)
    ...
    def yield_test_questions_K_edges(self, resampled=False, K=1, subset=False, expand_outfit=False):
        ...
        for question in questions:
            ...
            if subset:
                outfit_subset = np.random.choice(question[0], 9, replace=False)
            else:
            ...
            if K > 0:
                ...
                nodes_to_expand = choices_ids[:10]
    ...
    def setup_test_compatibility(self, resampled=False):
        ...
        if resampled:
            print("HARDERRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR")
