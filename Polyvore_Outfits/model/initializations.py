...
def mask_initial(num_attribute=10, input_dim=350, variable=False):

    mask_len = input_dim/num_attribute
    ...
    for i in range(num_attribute):
        ...
        for j in range(num_attribute):
            if j==i:
                mask_single.append(tf.ones([mask_len,]))
            else:
                mask_single.append(tf.zeros([mask_len,]))
        mask_array.append(tf.concat(mask_single,axis=0))

    if variable == False:
        return tf.stack(mask_array)
    return tf.Variable(tf.stack(mask_array))

def mask_random_normal(num_attribute=10, input_dim=350,variable=False):
    initial = tf.random_normal([num_attribute,input_dim],mean=0.9,stddev=0.7)
