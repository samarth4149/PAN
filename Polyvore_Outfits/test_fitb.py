...
def test_fitb(args):
    ...
    train_features, adj_train, _, _, _ = dl.get_phase('train')
    ...
    adj_q, q_r_indices, q_c_indices, q_labels, q_ids, q_valid = dl.get_test_questions(args.resampled)
    ...
    #                train_labels,None, train_r_indices, train_c_indices, config['dropout'],edge_c=False)
    val_feed_dict = construct_feed_dict(placeholders, val_features, val_support,
                        val_labels,None, val_r_indices, val_c_indices, 0., is_train=BN_AS_TRAIN,edge_c=False)
    test_feed_dict = construct_feed_dict(placeholders, test_features, test_support,
                        test_labels,None, test_r_indices, test_c_indices, 0., is_train=BN_AS_TRAIN,edge_c=False)
    q_feed_dict = construct_feed_dict(placeholders, test_features, q_support,
                        q_labels,None, q_r_indices, q_c_indices, 0., is_train=BN_AS_TRAIN,edge_c=False)
    ...
    with tf.Session() as sess:
        ...
        if args.resampled:
            ...
            num_option=4
        print("num_option is"+str(num_option))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-resampled', '--resampled', default=False,
